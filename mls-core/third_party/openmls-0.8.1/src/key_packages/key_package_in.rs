...
use crate::{
    ...
    credentials::*,
    extensions::{AnyObject, ExtensionType, Extensions},
...
impl KeyPackageIn {
    ...
    pub fn validate(
        ...
        // RFC 9420 s10 asks a KeyPackage's *creator* to include the types of
        // extensions it uses in leaf_node.capabilities. The receiver's own
        // obligations are narrower, and pulling them apart matters here:
        //
        //   s10   "unknown extensions in KeyPackage.extensions MUST be ignored,
        //          and the creator of a KeyPackage object SHOULD include some
        //          random GREASE extensions to help ensure that other clients
        //          correctly ignore unknown extensions."
        //   s13.4 "A client processing a KeyPackage object MUST ignore ... all
        //          unknown extensions in the extensions and leaf_node.extensions
        //          fields."
        //   s13.5 lists KeyPackage.extensions among the fields a sender SHOULD
        //          fill with GREASE, and its note requires reflection in
        //          capabilities only for LeafNode.extensions, "since the LeafNode
        //          validation process described in Section 7.3 requires that
        //          these two fields be consistent".
        //
        // Rejecting a KeyPackage over an undeclared unknown extension therefore
        // fails exactly the extensibility test GREASE exists to detect. Upstream
        // did that, and it rejected roughly seven in eight real mlspp KeyPackages,
        // whose GREASE extension is chosen independently of the capability list.
        //
        // Unknown types are skipped as a class - s13.5 forbids "any special
        // processing rules" for GREASE, so GREASE is not named here; it is simply
        // unknown. Recognised types stay bound to s10, so an undeclared
        // last_resort is still refused. leaf_node.extensions are untouched: they
        // remain checked against capabilities by LeafNode::validate_locally,
        // which is what s7.3 and the s13.5 note require.
        for extension in key_package.payload.extensions.iter() {
            let extension_type = extension.extension_type();
            if matches!(extension_type, ExtensionType::Unknown(_)) {
                continue;
            }
            if !key_package
                .payload
                .leaf_node
                .supports_extension(&extension_type)
            {
